...
vec_a
```
The result replaces 8 and 10 with vec_d. R wants vectors of the same length, so when it adds two vectors of different lengths, it recycles the smaller length vector until it's the same length as the longest length vector then it adds the vectors. R gives a warning that longer object length is not a multiple of shorter object length
...
vec_a
```
The result is R adding 5 to each element in the vector. It doesnt give an error because ur adding a constant to each element.
...
The difference between the two is that the first identifies NA values and 
selects rows without NA while the second one directly identifies row with NA. 
...
my.test <- list(X=x,Y=y,Slope=slope, PValue=p.value)
str(my.test)
...
```{r}
Example5 <- read_excel('Example_5.xls', sheet = 'RawData', skip = 4, 
                       range = "A5:C36")
...
```{r}
Example3 <- read_excel('Example_3.xls', sheet = 'data', range = 'A1:L34', 
                       na = c('-9999', 'NA'))
